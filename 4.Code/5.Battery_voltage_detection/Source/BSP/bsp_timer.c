...
void TIM1_Int_Init(u16 arr,u16 psc)
{
   TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
	...
	//  TIM1_Int_Init
	TIM_TimeBaseStructure.TIM_Period = arr; //Set the prescaler value is used to the TIMx clock frequency divisor
...
void TIM1_UP_IRQHandler(void)  
{
	...
	if (TIM_GetITStatus(TIM1, TIM_IT_Update) != RESET)  
	{
		...
		{
			GPIO_SetBits(Servo_J1_PORT, Servo_J1_PIN );		//Give the servo interface a high 
		}
		else
		{
			GPIO_ResetBits(Servo_J1_PORT, Servo_J1_PIN );		 //Give the servo interface a lo
		}
		#endif	   	
